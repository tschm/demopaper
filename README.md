# 📄 [Demo](https://tschm.github.io/demopaper/document.pdf)
...
2. **Header Configuration**:
   ...
   \fancyhead[L]{%
     \parbox[t]{0.8\textwidth}{ % Create a paragraph box that takes 80% of text width
       \raggedright\ttfamily\small\textcolor{blue}{Repo: \reponame, Latest Tag: \tagname, sha: \commitsha}
       % Left-aligned, monospace, small blue text
     }%
   }
...
In both the `latex.yml` and `release.yml` workflows, we use the `make_header.sh` script to generate a `header.tex` file
that contains detailed Git metadata including:
...
- Latest tag
- Commit date
- Links to the commit and tag on GitHub

This metadata is automatically included in the document header through LaTeX commands defined in the header.tex file,
making it easy to:
...
- Link directly to the specific commit or tag on GitHub

#### **Custom Headers**

For local development, you can run the `make_header.sh` script manually to generate a `header.tex` file with your
current Git metadata:

```bash
./make_header.sh
cp header.tex paper/header.tex
```

Alternatively, you can create your own custom `header.tex` file in the paper directory that defines the required
LaTeX commands:

```latex
% Example custom header.tex
\newcommand{\commitsha}{\texttt{abc1234}}
\newcommand{\branchname}{\texttt{main}}
\newcommand{\commitdate}{\texttt{2023-01-01}}
\newcommand{\reponame}{\texttt{username/repo}}
\newcommand{\tagname}{\texttt{v1.0.0}}
```
...
A basic `header.tex` file is required in the repository. This file is used as a placeholder
and is replaced during the build process in GitHub Actions workflows. When building locally,
if you don't run the `make_header.sh` script, you should ensure that the placeholder
file defines all the commands used in the document:

```latex
% Minimal header.tex for local development
\newcommand{\commitsha}{\texttt{local}}
\newcommand{\branchname}{\texttt{local}}
\newcommand{\commitdate}{\texttt{local}}
\newcommand{\reponame}{\texttt{local}}
\newcommand{\tagname}{\texttt{local}}
```
